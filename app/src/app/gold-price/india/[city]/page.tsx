...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  if (!city) return {}
  const karat = k === '22' ? 'gold_22k' : 'gold_24k'
...
export default async function GoldPriceCityPage({ params, searchParams }: Props) {
  const { city: citySlug } = await params
  const { k } = await searchParams
  const city = getMetalCityBySlug(citySlug)
  if (!city) notFound()
  ...
  const karat = k === '22' ? 'gold_22k' : 'gold_24k'
  const karatLabel = k === '22' ? '22K Gold' : '24K Gold'
  ...
  const tabs: Tab[] = [
    ...
    { subtype: 'gold_22k', label: '22K Gold', href: `/gold-price/india/${citySlug}?k=22` },
